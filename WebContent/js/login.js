...
$(document)
		.ready(
				function() {
					...
					// ----------------------> Login Click
					// Traitement si les champs sont vides.
					 but_login.click(
									function(e) {
										...
										if (document.formLogin.username.value == ""
											...
											alertMessage("error",
													"Il faut saisire Nom d'utilisateur et mot de passe");
											$('.inner').jrumble({
												x : 4,
												y : 0,
												rotation : 0
...
function alertHide() {
	$('#alertMessage').each(function(index) {
		$(this).attr("id", "alertMessage" + index).animate({
			opacity : 0,
			right : '30'
		}, 500, function() {
			$(this).remove();
...
function alertMessage(type, str) {
	...
	$('body').append(
			'<div id="alertMessage" class="alertMessage ' + type + '">');
